...
const _hash   = (x: number, y: number) => { const s = Math.sin(x * 127.1 + y * 311.7) * 43758.5453; return s - Math.floor(s); };
...
const _noise  = (x: number, y: number) => {
  ...
  return _lerp(_lerp(_hash(ix, iy), _hash(ix + 1, iy), fx), _lerp(_hash(ix, iy + 1), _hash(ix + 1, iy + 1), fx), fy);
};

export function buildNoiseTexture(): HTMLCanvasElement {
  const NTEX = 96;
  const canvas = document.createElement("canvas");
  canvas.width = canvas.height = NTEX;
  const ctx   = canvas.getContext("2d")!;
  const ndata = ctx.createImageData(NTEX, NTEX);
  for (let ny = 0; ny < NTEX; ny++) {
    for (let nx = 0; nx < NTEX; nx++) {
      const sx = nx / NTEX, sy = ny / NTEX;
      const n = _noise(sx * 4, sy * 4) * 0.50 + _noise(sx * 9, sy * 9) * 0.32 + _noise(sx * 20, sy * 20) * 0.18;
      const v = Math.round((n - 0.5) * 90);
      const i = (ny * NTEX + nx) * 4;
      ndata.data[i] = ndata.data[i + 1] = 128 + v;
...
export function drawMoon(ctx: CanvasRenderingContext2D, rotation: number, noiseCanvas: HTMLCanvasElement): void {
  const baseGrad = ctx.createRadialGradient(CX + R * 0.32, CY - R * 0.32, R * 0.08, CX, CY, R);
  ...
  for (const mare of MARE) {
    const lon = mare.lon + rotation, cosLat = Math.cos(mare.lat);
    const x3d = cosLat * Math.cos(lon), y3d = Math.sin(mare.lat), z3d = cosLat * Math.sin(lon);
    if (z3d < -0.1) continue;
    const px = CX + x3d * R, py = CY - y3d * R;
    const scale = Math.max(0, (z3d + 1) / 2);
    const mrx = mare.rx * R * scale * cosLat, mry = mare.ry * R * scale;
  ...
  for (const crater of CRATERS) {
    const lon = crater.lon + rotation, cosLat = Math.cos(crater.lat);
    const x3d = cosLat * Math.cos(lon), y3d = Math.sin(crater.lat), z3d = cosLat * Math.sin(lon);
    if (z3d < -0.15) continue;
    const px = CX + x3d * R, py = CY - y3d * R;
    const scale = Math.max(0, (z3d + 1) / 2);
    const cr = crater.radius * scale * cosLat;
    if (cr < 2) continue;
    const floorGrad = ctx.createRadialGradient(px + cr * 0.25, py + cr * 0.25, 0, px, py, cr);
  ...
  const lightGrad = ctx.createRadialGradient(CX + R * 0.38, CY - R * 0.38, 0, CX, CY, R);
